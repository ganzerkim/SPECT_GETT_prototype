    ...
    if detector == "ANT":
        ...
        for i in range(len(dcm_tmp_2)):
            ...
            if position_1 == 'ANT':
                ...
                img_geo.append(np.sqrt(ccc[0] * np.flip(ccc[1], 1)))
                
            elif position_1 == 'POST':
                img_list.append((ccc[1] + np.flip(ccc[0], 1))/2)
                img_geo.append(np.sqrt(ccc[1] * np.flip(ccc[0], 1)))
        ...
        for i in range(len(dcm_tmp_2)):
            ...
            if position_1 == 'ANT':
                ...
                img_geo.append(np.sqrt(np.flip(ccc[0], 1) * ccc[1]))
                
            elif position_1 == 'POST':
                img_list.append((np.flip(ccc[1], 1) + ccc[0])/2)
                img_geo.append(np.sqrt(np.flip(ccc[1], 1) * ccc[0]))
...
msk_img = get_msk_img(img_geo, roi_order) #if geometry correction is on
#msk_img = get_msk_img(img_list, roi_order) #if geometry correction is off
...
def countcal(roi_number, dcm_tmp_2):
    ...
    for i in range(len(dcm_tmp_2)):
        ...
        m_cps = roi_cps(msk_img[i][roi_number], roi_order[roi_number])
        f_cps = roi_cps(msk_img[0][roi_number], roi_order[roi_number])
        y.append(m_cps)
    
        empty.append((f_cps - m_cps)/f_cps * 100)
        background.append((np.sum(msk_img[i][2])/60))
        
    return x, y, empty, background

x, y, empty, background = countcal(0, dcm_tmp_2)
x2, y2, empty2, background2 = countcal(1, dcm_tmp_2)
...
def polynominalfit(x, y, x2, y2):
    ...
    plt.axvline(halflife, color = 'Orange', linestyle = ':', label ="99m-Tc T(1/2)", linewidth = 1)
    plt.legend()
    ...
    return fit, fit2, MSE, RMSE, Rsquared, MSE2, RMSE2, Rsquared2
    
fit, fit2, MSE, RMSE, Rsquared, MSE2, RMSE2, Rsquared2 = polynominalfit(x, y, x2, y2)


def savedataset(dcm_p, x, y, x2, y2, halflife, empty, empty2, background, background2, correcty, correcty2, fit, fit2, RMSE, Rsquared, RMSE2, Rsquared2):
...
savedataset(dcm_p, x, y, x2, y2, halflife, empty, empty2, background, background2, correcty, correcty2, fit, fit2, RMSE, Rsquared, RMSE2, Rsquared2)
